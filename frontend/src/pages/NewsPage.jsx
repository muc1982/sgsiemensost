...
import { getNews } from "@/services/contentful";
...
export default function NewsPage() {
  ...
  useEffect(() => {
    const loadNews = async () => {
      setLoading(true);
      try {
        // Versuche News von Contentful zu laden
        const contentfulNews = await getNews();
        
        if (contentfulNews && contentfulNews.length > 0) {
          setNews(contentfulNews);
        } else {
          // Fallback zu statischen Daten wenn keine Contentful-Daten vorhanden
          setNews(NEWS_DATA);
        }
      } catch (error) {
