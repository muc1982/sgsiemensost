...
export const PlayerCard = ({ player, variant = "default" }) => {
  ...
  const cardClasses = {
    ...
    captain: "col-span-2 row-span-1 md:col-span-2",
    goalkeeper: "col-span-1 row-span-2",
  };
    ...
    >
      ...
      {number && (
        <div className="absolute top-3 right-3 z-10">
          <span className="font-accent text-4xl font-bold text-cyber-gold/80 group-hover:text-cyber-gold transition-colors duration-300">
            {number}
          </span>
        </div>
      )}
      ...
      {/* Captain Badge */}
      {is_captain && (
        <div className="absolute top-3 left-3 z-10 bg-cyber-gold text-midnight-pitch px-2 py-1 rounded text-xs font-heading uppercase">
          Kapitän
        </div>
      )}

      {/* Player Image */}
      <div className={`relative ${currentVariant === 'goalkeeper' ? 'h-64' : 'h-48'} overflow-hidden bg-slate-800`}>
        {image_url ? (
          <img 
            src={image_url} 
            alt={name}
      ...
      <div className="p-4 relative z-10">
        ...
        <p className="text-slate-400 text-sm mt-1">{position}</p>
      </div>
    </div>
  );
};
...
export const SquadGrid = ({ players = [], title = "Kader" }) => {
  ...
  return (
    <section className="py-16" data-testid="squad-grid">
      <h2 className="font-heading text-3xl md:text-4xl text-white mb-8">{title}</h2>
...
export const CoachesGrid = ({ coaches = [], title = "Trainerteam" }) => {
  return (
    <section className="py-16" data-testid="coaches-grid">
      <h2 className="font-heading text-3xl md:text-4xl text-white mb-8">{title}</h2>
